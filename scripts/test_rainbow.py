...
N_ATOMS = 51  # Number of discrete atoms for the value distribution

V_MIN = -10  # Minimum possible return
V_MAX = 10   # Maximum possible return
...
def test(envs, n_actions, path):    
    ...
    dqn = RainbowDeepQNetwork(n_actions).to(device)
    ...
    while total_episodes < TEST_EPISODES:
        with torch.no_grad():
            ...
            if np.random.rand() < eps:
                ...
                q_atoms = dqn(
                    torch.tensor(np.array(t_observations), device=device)
                )
                q_values = (q_atoms * torch.linspace(V_MIN, V_MAX, N_ATOMS, device=device)).sum(dim=2)
        ...
        for i in range(NUM_ENVS):
            if dones[i]:
                # print(f"Env {i} finished episode {episode_counts[i]} with reward {episode_rewards[i]}")
...
if __name__ == "__main__":
    ...
    checkpoint_files = glob.glob("./rb_assault_checkpoint*.pt")
    ...
    envs = gym.vector.AsyncVectorEnv([make_env("ALE/Assault-v5", seed=RANDOM_SEED + idx) for idx in range(NUM_ENVS)])
    n_actions = envs.single_action_space.n
    ...
    df.to_csv('test_rb_assault_results.csv', index=False)
    ...
    plt.ylabel('Reward')
    plt.title('Performance of RDQN on Assault-v5')
    plt.legend()
    plt.grid(True)
    plt.show()
